---
title: "Javaメモ: コールスタックを表示する (Throwable#getStackTrace)"
url: "p/uep5gr9/"
date: "2010-07-29"
tags: ["java"]
aliases: ["/java/call-stack.html"]
---

任意の場所でコールスタックを表示する
----

`Throwable` オブジェクトの `getStackTrace` メソッドを使用して、現在のコンテキストにおけるスタックトレース情報を取得することができます。

{{< code lang="java" title="Test.java" >}}
public class Test {
    public void method1() {
        method2();
    }
...
出力フォーマットにこだわりがないのであれば、単純に、`Throwable#printStackTrace()` を使用して標準エラー出力に情報を出力することもできます。
...
この場合は、コールスタックを表示するためのメソッド自身の情報を表示しないように、表示開始位置を 1 つずらしておく必要があります。

{{< code lang="java" title="CallStackUtil.java" >}}
...
{{< code lang="java" title="Log.java より抜粋" >}}
