---
title: "JavaScriptメモ: 配列に要素を追加・挿入する (push, unshift, splice)"
url: "p/75v64mq/"
date: "2018-09-10"
tags: ["javascript"]
aliases: [/js/array/push.html]
---

配列に要素を追加するには、追加する位置によって下記のようなメソッドを使い分けます。

- **`Array#push()`**: 末尾に要素を追加。
- **`Array#unshift()`**: 先頭に要素を追加。
...
配列オブジェクトの末尾に任意の要素を追加するには、`Array#push(elem, ...)` メソッドを使用します。
...
第1パラメータの挿入位置には、負の値で末尾からの位置を指定することも可能です。
