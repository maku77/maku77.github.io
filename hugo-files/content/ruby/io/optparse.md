---
title: "Rubyメモ: コマンドライン引数によるオプションに対応する (optparse)"
url: "p/r2ryc4t/"
date: "2016-04-18"
tags: ["ruby"]
aliases: ["/ruby/io/optparse.html"]
---

## OptionParser の基本

`OptionParser` クラスを使用すると、`-a` や `--add` といった形式のコマンドラインオプションを簡単に扱えます。

`OptionParser` で処理すべきオプションは、下記の `on` メソッドで設定します。
...
下記の例では、`OptionParser` を使用して、`-a (--add)` オプションと、`-d (--delete)` オプションを処理しています。
...
`on` メソッドでオプション名を指定するときに、その後ろにパラメータ名を指定すると、そのオプションにはパラメータ指定が必要であることを示せます。
...
次のようにオプションごとにパラメータを受け取れるようになります。
...
この例のように、各オプションに対してパラメータを単純に追記すると、そのパラメータの指定は必須とみなされます。
例えば、下記のようにパラメータの指定を忘れるとエラーが発生します。
...
これらのハンドラの中では、`OptionParser` オブジェクト自体を出力するようにすると、ヘルプメッセージを表示できます。
...
最後に指定した `hoge` という引数は、オプションのパラメータとはみなされないため、`OptionParser#on('-n' ...)` によるハンドラの中で取得できません。
...
`OptionParser` クラスを使用して、そのアプリケーションに必要なコマンドライン引数をパースするためのクラスを作成しておくと、オプションを扱うコードの見通しがよくなります。
