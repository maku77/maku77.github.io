...
### 表示の設定
* [Vim/Neovim で行番号を表示する (`number`)](/p/t8o6tum/)
* [Vim/Neovim で80文字目に縦線を表示する (`colorcolumn`)](/p/ngz6n6i/)
* [Vim/Neovim で構文強調（シンタックスハイライト）を有効にする (`syntax`)](/p/wcfbnqj/)
* [Vim/Neovim でカーソル位置の行や列をハイライト表示する (`cursorline`, `cursorcolumn`)](/p/osrx94g/)
* [Vim/Neovim で GUI モード用のフォントを設定する (`guifont`)](/p/e3xdbxe/)
* [制御文字（改行、タブ文字、行末のスペースなど）を表示する (`list`, `listchars`)](/p/s596qii/)
* [Vim/Neovim で全角スペースを見えるように表示する](/p/preoa93/)
* [Vim/Neovim でステータスラインの表示内容を設定する (`laststatus`, `statusline`)](/p/oegfris/)
* [Vim で■や※などの記号が重なって表示される問題の解決 (`ambiwidth`)](/p/fp2xk7y/)
* [GVim 起動時のウィンドウの幅、高さを設定する (`columns`, `lines`)](/p/syn3cyx/)

### 入力に関する設定
* [Vim/Neovim で OS のクリップボードを連動させる (`clipboard`, `unnamed`, `unnamedplus`)](/p/nnhefs3/)
* [Vim/Neovim でタブ文字に関する設定を行う (`tabstop`, `expandtab`, `softtabstop`)](/p/8okf7d3/)
* [Vim/Neovim でインデント（シフトコマンド）を設定する (`shiftwidth`, `shiftround`)](/p/b5o6ksu/)
* [Vim/Neovim で自動インデントモードを有効にする (`autoindent`, `smartindent`, `cindent`)](/p/oe94dkh/)
* [Vim/Neovim でビジュアルモード中に連続インデントできるようにする](/p/hoihkfy/)
