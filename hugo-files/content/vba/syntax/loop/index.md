...
`Do ~ Loop` によるループは、無限ループになるので、何らかの条件を満たしたときに `Exit Do` を明示的に呼び出してループを抜ける必要があります。
...
{{< code title="実行結果（イミディエイトウィンドウへ出力されます）" >}}
Count = 0
Count = 1
Count = 2
ループを抜けます
...
{{< code lang="vb" title="1 から 5 までのループ" >}}
Sub Test()
    Dim i As Integer
    For i = 1 To 5
...
{{< code lang="vb" title="3, 2, 1, 0 とカウントダウン" >}}
Sub CountDown()
    Dim i As Long
    For i = 3 To 0 Step -1
        Debug.Print i
