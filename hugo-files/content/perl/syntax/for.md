---
title: "Perlメモ: for/foreach によるループ処理"
url: "p/8pod578/"
date: "2008-03-16"
tags: ["perl"]
aliases: ["/perl/syntax/for.html"]
---
...
{{< code lang="perl" title="以下は両方とも同じ foreach ループとして処理される" >}}
print for 1..10;
print foreach 1..10;
{{< /code >}}
...
{{< code lang="perl" title="break でループを抜ける" >}}
for (1..10) {
    print "$_\n";
...
{{< code lang="perl" title="next で直ちに次の要素を処理する" >}}
for (1..10) {
...
{{< code lang="perl" title="redo で現在の要素をもう一度処理する" >}}
for (1..10) {
    print "$_\n";
    print "Redo? (Y/N): ";
    chomp (my $line = <STDIN>);
...
{{< code lang="perl" title="外側のループを制御する" >}}
LINE: while (<>) {
    for (split) {
