---
title: "Perlメモ: インポート可能なモジュールを作成する"
url: "p/3i77yak/"
date: "2008-06-29"
tags: ["perl"]
aliases: ["/perl/module/create-module.html"]
---
...
{{< code lang="perl" title="MyLib.pm（独自モジュール）" >}}
package MyLib;
...
{{< code lang="perl" title="main.pl（使用例）" >}}
use MyLib;
func1;
{{< /code >}}
...
つまり、`MyLib` パッケージの中で適切に `import` サブルーチンを実装することで、main パッケージにサブルーチンをインポートできるようになります。
...
Exporter モジュールは `import` ルーチンを提供しているので、これを継承するようにして独自のモジュールを作成します。`Exporter->import` には次のような特徴があります。
...
デフォルトのインポートリストに加え、さらに明示的にインポートするサブルーチンを指定する場合は、デフォルトのインポートリストを示す `:DEFAULT` というショートカットを使用すると便利です。
