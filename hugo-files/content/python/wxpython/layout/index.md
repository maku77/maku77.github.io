---
title: "Pythonメモ: wxPython - Layout（レイアウト）"
url: "p/e4c6ef2/"
date: "2007-03-15"
tags: ["python"]
aliases: /python/wxpython/layout.html
---
...
次の例では、1 つ目のボタンと 2 つ目のボタンを、`wx.BoxSizer` で並べた方向と垂直な方向に伸ばして表示しています。
...
{{< image src="img-013.png" >}}
{{< image src="img-014.png" >}}
...
{{< image src="img-015.png" >}}
{{< image src="img-016.png" >}}
...
wx.GridBagSizer で複数行／列にまたがる widget を配置する
----

`wx.FlexGridSizer` を拡張し、ひとつの widget が複数の区画をまたがるように配置できるようにしたのが `wx.GridBagSizer` です。
`wx.GridBagSizer` では、`Add()` メソッドで widget を配置することで自動的に最終的なレイアウトを決定するので、コンストラクタではそのサイズを指定する必要はありません。

```python
sizer = wx.GridBagSizer([各行の間隔], [各列の間隔])
```

`wx.GridBagSizer#Add()` メソッドでは、追加する widget の他に、追加する位置 (`wx.GBPosition`)、サイズ (`wx.GBSpan`) を指定する必要があります。
...
{{< image src="img-017.png" >}}
{{< image src="img-018.png" >}}
