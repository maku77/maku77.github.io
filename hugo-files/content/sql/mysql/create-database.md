---
title: "SQLメモ: MySQL で新しいデータベースを作成する (CREATE DATABASE)"
url: "p/dwzbc8b/"
date: "2012-07-14"
tags: ["sql"]
aliases: /sql/mysql/create-database.html
---

ユーザ testuser から操作可能なデータベース testdb を作成する
----

まずは、ユーザ `testuser` にデータベース `testdb` を操作できる権限を与えます。
...
上記の例では `Enter password:` プロンプトからパスワードを入力していますが、`mysql` コマンドのパラメータで直接パスワードを指定するには、以下のように `-p` オプションの直後に空白を入れずに指定します。

```console
$ mysql -u testuser -ptestpass testdb
```

以下のように `alias` 設定を行っておくと、簡単にデータベースに接続できるようになります。
