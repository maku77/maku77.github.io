---
title: "Windowsメモ: Excel で順位セルの値通りに並び替えて表示する"
url: "p/t2uzpn2/"
date: "2018-11-13"
tags: ["windows"]
aliases: /windows/excel/rank-sort.html
---

Excel の `RANK.EQ` 関数を使用すると、点数などを基準に順位を求めることができます。
例えば、下記の順位のセルは、左の得点セルの値をもとに自動的に計算されています（Excel 関数によって自動計算されるセルは灰色で表示しています）。

{{< image src="img-001.png" >}}

- 参考: [Excel で点数などから順位を求める（RANK.EQ 関数）](/p/m9znv3s/)
...
この補正を入れることで、表示上は四捨五入されて同じ順位だけど、内部的には数値が微妙に異なるという状態を実現できます。
