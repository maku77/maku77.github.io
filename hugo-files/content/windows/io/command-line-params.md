---
title: "Windowsメモ: バッチファイルでコマンドライン引数を扱う"
url: "p/s7r9r9p/"
date: "2019-10-02"
tags: ["windows"]
...
{{< code lang="bat" title="例: 1番目のパラメータが指定されていることを確認する" >}}
if not "%~1"=="" (
    echo %1
)
{{< /code >}}
...
{{< code lang="bat" title="例: 1番目のパラメータが指定されていないことを確認する" >}}
if "%~1"=="" (
...
{{< code lang="bat" title="例: 3 つのパラメータが渡されていなければバッチファイルを終了する" >}}
if "%~3"=="" (
...
{{< code lang="bat" title="例: 渡されたパラメータを 1 つずつ echo する" >}}
@echo off
FOR %%a IN (%*) DO echo %%a
{{< /code >}}
