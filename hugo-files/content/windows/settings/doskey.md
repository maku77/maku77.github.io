---
title: "Windowsメモ: doskey を使ってコマンドプロンプト用のマクロを登録する"
url: "p/3u7yuu2/"
date: "2014-10-04"
lastmod: "2020-10-22"
tags: ["windows"]
aliases: /windows/settings/doskey.html
---
...
現在登録されているマクロの一覧を表示するには **`doskey /macros`** コマンドを使用します。
...
コマンドプロンプト (`cmd.exe`) の起動時に、**`/k`** オプションを使用すると、任意のコマンドを実行した上で起動することができます。
...
`cmd.exe` のショートカットを作成し、そのプロパティの **リンク先** を下記のように変更します（`/k` オプション以降を追加）。
