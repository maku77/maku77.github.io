---
title: "Windowsメモ: カレントディレクトリ以下のファイルやディレクトリを順に処理する (for /r)"
url: "p/nmp2o4d/"
date: "2010-04-25"
tags: ["windows"]
aliases: /windows/for-each-file.html
---
...
{{< code title="例: カレントディレクトリ以下の java ファイルのフルパスを列挙する" >}}
C:\> FOR /R . %x IN (*.java) DO @echo %x
{{< /code >}}

{{< code title="例: カレントディレクトリ以下の java ファイルを出力する" >}}
C:\> FOR /R . %x IN (*.java) DO @type %x
{{< /code >}}

{{< code title="例: カレントディレクトリ以下の java ファイルの行数を調べる" >}}
...
{{< code title="例: build ディレクトリをまとめて削除する（中身もすべて削除）" >}}
...
* 参考: [DIR コマンドでファイルやディレクトリを再帰的に検索する (`dir /b`)](/p/chexxgu/)
