---
title: "Windowsメモ: バッチファイルでサブルーチンを定義する、別のバッチファイルを呼び出す (call, setlocal)"
url: "p/jz8fwmk/"
date: "2015-02-09"
lastmod: "2021-03-12"
tags: ["windows"]
aliases: /windows/subroutine.html
---
...
先頭の `%~dp0` は、バッチファイルが含まれているディレクトリを表すおまじないです（参考: [バッチファイルのファイル名やディレクトリ名を取得する](/p/76ipisd/)）。
...
{{< code lang="bat" title="Main.cmd（呼び出す側）" >}}
@echo off && setlocal
...
{{< code lang="bat" title="SubModule.cmd（呼び出される側）" >}}
@echo off && setlocal
