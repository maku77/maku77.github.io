---
title: "Windowsメモ: .NET - 現地時刻（ローカルタイム）と世界協定時刻 (UTC) の扱いを理解する"
url: "p/pmua6r4/"
date: "2009-03-07"
tags: ["windows"]
aliases: /windows/dotnet/localtime-and-utc.html
---
...
`Kind` プロパティが `Local` の場合は現地時刻を表す文字列に変換され、`Utc` の場合は世界標準時を表す文字列に変換されます。
...
`Kind` プロパティの値を変更しても、内部で保持している値が local time なのか UTC なのかを区別するために使われるだけなので、文字列表現への変換結果は変わりません。
