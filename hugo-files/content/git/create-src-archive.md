---
title: "Gitメモ: ソースコードのアーカイブファイル (ZIP/TAR) を作成する (git archive)"
url: "p/dmpa5vw/"
date: "2010-07-17"
tags: ["git"]
aliases: [/git/create-src-archive.html]
---

最新のソースコードのアーカイブを作成する
----

**`git archive`** コマンドを使用すると、現在チェックアウトしているブランチの最新コミット (HEAD) のコードを ZIP や TAR アーカイブファイルとしてバックアップできます。
バックアップには `.git` ディレクトリ（リポジトリ本体）は含まれないため、あくまでソースコードを配布するときなどに使用できるコマンドです。

{{< code lang="console" title="例: ZIP ファイルを作成する" >}}
...
{{< code lang="console" title="例: tar.gz ファイルを作成する" >}}
