---
title: "Gitメモ: Git で変更内容にある文字列が含まれているコミットを検索する (git log -G/-S)"
url: "p/hf7bnyi/"
date: "2020-12-22"
tags: ["git"]
aliases: [/git/log/pickaxe.html]
---
...
{{< code lang="console" title="例: 変更内容に xxxYyyZzz が含まれているコミットを探す" >}}
$ git log -G"xxxYyyZzz"
{{< /code >}}
...
__`-S`__ オプションは、`-G` オプションとはちょっと異なり、指定した単語で検索したときの __追加行と削除行の数が異なる__ コミットのみ検索します。
つまり、`-S` オプションの方を使えば、ある名前の関数が初めて追加されたコミットだけを効率的に探すことができます（リファクタで関数を移動しただけの場合は、追加行と削除行の数が等しくなるのでヒットしなくなる）。

{{< code lang="console" title="例: 初めて xxxYyyZzz が追加されたコミットを探す" >}}
$ git log -S"xxxYyyZzz"
{{< /code >}}
...
- 参考: [コミット時のコメントを grep する (`git log --grep`)](/p/hi5iir5/)
