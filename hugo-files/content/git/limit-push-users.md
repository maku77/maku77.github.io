---
title: "Gitメモ: 特定のユーザのみ git push できるように制限する"
url: "p/f84ekdq/"
date: "2010-08-26"
tags: ["git"]
aliases: [/git/limit-push-users.html]
---
...
{{< code title=".git/hooks/authorized_users" >}}
# Authorized Users
user1  # Jack
user2  # Smith
user3  # Justine
...
{{< code lang="bash" title=".git/hooks/update" >}}
#!/bin/sh
...
{{< code lang="bash" title=".git/hooks/authorized_users（Submit コントロールをかけない場合）" >}}
# authorized_users の中身を空にする（コメントは記述してよい）
{{< /code >}}

{{< code lang="bash" title=".git/hooks/authorized_users（master ブランチは user1, user2, user3 にだけ push を許可する）" >}}
[refs/heads/master]
user1
user2
user3
...
{{< code lang="bash" title=".git/hooks/authorized_users（master ブランチは全員 push 不可）" >}}
[refs/heads/master]
# ブランチ名だけ上記のように指定し、ユーザ名を記述しない。
{{< /code >}}

{{< code lang="bash" title=".git/hooks/authorized_users（master ブランチは全員 push 不可、maint ブランチは user1 だけ push 許可）" >}}
[refs/heads/master]
...
{{< code lang="bash" title="参考: デフォルトの .git/hooks/authorized_users" >}}
#
# Authorized users
#
