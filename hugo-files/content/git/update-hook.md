---
title: "Gitメモ: Git の update フックの基本 (.git/hooks/update)"
url: "p/gyotfx7/"
date: "2010-08-23"
tags: ["git"]
aliases: [/git/update-hook.html]
---

**`.git/hooks/update`** スクリプトは、クライアントが `git push` しようとしたときに、サーバ側で実行されるフックスクリプトです。
このスクリプトが `0` 以外の値を返すと、`git push` は失敗するようになっています。

例えば、以下のような shell スクリプトを push される側の `.git/hooks/update` として置いて実験してみましょう。

{{< code lang="bash" title="例: .git/hooks/update スクリプト" >}}
echo "--------------------------------------------------"
echo USER = $USER
echo refname = $1
echo old rev = $2
