---
title: "Gitメモ: あるバージョン間の変更行数（追加＆削除）を集計する (git diff --stat, git log --numstat)"
url: "p/3cjkt3v/"
date: "2018-12-04"
tags: ["git"]
aliases: [/git/stats/count-changes.html]
---
...
{{< code lang="console" title="リビジョンの指定方法いろいろ" >}}
...
{{< code lang="console" title="例: カレントディレクトリ以下のみ" >}}
$ git diff --stat v1 v2 -- .
{{< /code >}}

ディレクトリパスなどを指定して絞り込みを行う場合は、タグ名などの指定とあいまいになるのを防ぐために、パスの指定の前に `--` を入れます（上記の例では省略可能です）。

{{< code lang="console" title="例: カレントディレクトリ以下の java ファイルのみ" >}}
$ git diff --stat v1 v2 -- "./*.java"
{{< /code >}}

{{< code lang="console" title="例: カレントディレクトリ以下のみ、ただし xml ファイルを除く" >}}
$ git diff --stat v1 v2 -- . ":(exclude)*.xml"
{{< /code >}}


(2) git log --numstat を使う方法
----
...
{{< code lang="console" title="v2 にだけ入っている変更の一覧と、それぞれのファイルの変更行数を出力" >}}
...
実際に変更行数（追加行数と削除行数）を集計したい場合は、上記のような出力だとやりにくいので、出力フォーマットを指定する **`--pretty`** オプションを追加することで、ファイルの変更行数だけをリスト出力するとよいでしょう。
...
...
```
この出力をテキストファイルなどにリダイレクトしておいて、Excel などにその内容を貼り付けてやれば、追加行数と削除行数の合計は簡単に求めることができます。
...
33850 16243
```
- 参考: [Ruby でワンライナープログラミング](/p/f6fj4c7/)
