---
title: "Gitメモ: git-svn を使って Git と Subversion を連携する"
url: "p/mujnu3f/"
date: "2010-12-07"
tags: ["git"]
aliases: [/git/svn/git-svn.html]
---
...
$ git log svn/trunk
```
ただし、`git svn clone` した時点ではリモートトラッキングブランチが作成されるだけなので、ローカルファイルとしてコードを参照したい場合は、以下のような感じでローカルブランチを作成し、チェックアウトする必要があります。
...
git-svn を使用するのであれば、Subversion から Git に完全に移行してしまうのをおすすめしますが、何らかの理由で Subversion を使い続けなければならないこともあります。
...
このような作業を慎重に行いたいのであれば、まずは `--dry-run` オプションをつけて実行し、何が起こるかをシミュレートしてみるとよいでしょう。
