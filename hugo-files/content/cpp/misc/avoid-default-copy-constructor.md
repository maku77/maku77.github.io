---
title: "C++メモ: 値コピーできないメンバ変数を持つクラスでは、コピーコンストラクタと代入演算子をオーバーロードする"
url: "p/d9uymco/"
date: "2009-12-07"
tags: ["cpp"]
aliases: /cpp/misc/avoid-default-copy-constructor.html
---
...
メンバ変数の単純なコピーが行われては困るクラスでは、コピーコンストラクタ、代入演算子を定義する必要があります。
ポインタではなく、**代入演算子が定義されていないオブジェクトをメンバ変数として持っている場合も同様**です。
...
int main() {
    ...
    // b の破棄による "Destructor B" ⇒ 二重解放の可能性
}
```
