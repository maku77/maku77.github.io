...
※注: このスクリプトは `p4 diff` コマンドの出力を使用しています。
`p4 diff` で出力されない新規ファイルなどの内容は含まれないことに注意してください。
...
仮に近いものが作れたとしても、次のような理由により、他のユーザーはそのパッチファイルを適用できません。

* `p4 diff` コマンドには unified 形式の diff を出力する `-du` オプションがあるが、ファイルパスが Perforce の独自形式のパスになっている。
* 各ユーザーは P4 クライアント設定により、サーバ上のファイルを任意のローカルパスにマッピングできるため、全ユーザーが共通して使用できるファイルパスが存在しない。

そこで、ここでは、以下のようなアプローチで解決しています。

* パッチファイルを作成するときは独自スクリプトを使い、パッチファイル内のファイルパスには P4 サーバ上のファイルパス（P4 独自のパス形式）を出力するようにする。
このパッチファイルの拡張子を `.p4patch` とする。
* パッチファイル (`.p4patch`) を適用するときは、独自のスクリプトを使い、パッチファイル内のファイルパスをそのクライアントのローカルファイルパスに書き換えてから適用する。
...
そして、パッチファイルを適用するときに、`##` で始まる Perforce パスを、そのユーザーのローカルパスに変換してから適用します。
