---
title: "Perforceメモ: ブランチを作成する"
url: "p/toj2q3b/"
date: "2005-07-12"
tags: ["perforce"]
aliases: ["/p4/create-branch.html"]
---
...
まだこの時点では、サーバ上に `//depot/B/` は存在しないし、クライアント上にも対応するファイル群は作成されていません。
...
作業コピーとして作成されたファイルは、`//depot/B/...` というブランチを作成するための作業状態としてチェンジリストに追加されます。
...
`submit` コマンドを実行する時点では、`//$CLIENT/B/...` のファイルが新規作成の状態になっているため、これらのファイルがすべて `//depot/B/...` へサブミットされます。
これで、他のユーザーも `//depot/B/...` を参照してブランチの作業コピーを取得できるようになります。
