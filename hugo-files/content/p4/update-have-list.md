...
クライアントがローカルのワークスペースに持っているファイル情報 (Have List) はサーバ上で保持されているため、ワークスペース内のファイルをローカルで直接変更した場合は、その変更をサーバに通知しなければ次回以降の sync に影響が出てしまいます。
ローカルのファイル情報をサーバに通知するには、`p4 sync` コマンドに `-k` オプションを付けて実行します。
