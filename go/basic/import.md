...
Go 1.11 以降は「モジュール」という考え方が導入され、Go 1.10 以前の GOPATH によるコードの一元管理からの移行が進みました。
簡単に言うと、プロジェクト単位でディレクトリを自由な場所に置けるようになりました（昔は `$GOPATH` 以下に置くという奇妙な仕組みだった）。
...
同じパッケージ内の Go ファイルで定義されている関数は、（同時にビルドされる限り）互いに参照することができます。
...
`greet` 関数は、同じパッケージ（同じディレクトリ内）の別のファイルで定義しています。
...
同じモジュール内の別パッケージを参照する
----

次のようなディレクトリ構成の `myapp` モジュールを作って、`main.go` から `mymath/mymath.go` で定義した関数を参照したいとします。
...
ここではローカルだけで使うことを前提に、モジュールパスは `com.example/myapp` としましたが、GitHub での公開を想定しているのであれば、`github.com/<user>/<repo>` のように世界で一意となる名前を付けるべきです。
...
注意点としては、`"./mymath"` のような相対パスではインポートできないところです（少なくとも Go 1.18 時点では）。
インポート時には `"com.example/myapp/mymath"` のようにモジュール名を含むパッケージパス全体（絶対パス）を指定する必要があります。

`main` パッケージは次のように実行できます。
