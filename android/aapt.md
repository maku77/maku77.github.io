...
- `aapt d badging myapp.apk`
  - パッケージ名、アプリ名、バージョン、パーミッション情報、サポートする Locale の一覧などを確認
- `aapt d permissions myapp.apk`
  - 使用しているパーミッション (uses-permission) と、定義しているパーミッション (permission) の一覧
- `aapt d strings myapp.apk`
  - 文字列リソースの一覧
- `aapt d resources myapp.apk`
  - リソースの一覧（大量に出力されるので注意）
- `aapt d xmltree myapp.apk res/animator/fadein.xml`
  - 指定した XML リソースのツリー情報
- `aapt list myapp.apk`
  - リソースファイル名の一覧
